...
### Fixed
- Fixed compilation error: `NEHotspotNetwork.fetchCurrent()` returns optional `NEHotspotNetwork?`, not a collection

## [2.1.0] - 2025-11-14

### Fixed
- **CRITICAL**: Fixed iOS 26 compatibility by replacing deprecated `CNCopyCurrentNetworkInfo` with `NEHotspotNetwork.fetchCurrent()`
- `CNCopyCurrentNetworkInfo` was deprecated in iOS 26 and always returns `nil`, breaking WiFi detection
- Now uses modern NetworkExtension API for iOS 13+ (minimum supported iOS version is 15)

### Changed
- iOS: Replaced `CNCopyCurrentNetworkInfo` with `NEHotspotNetwork.fetchCurrent()` from NetworkExtension framework
- iOS: Updated `getCurrentWifiInfo()` to use async/await pattern for modern API
- Improved error handling for network information retrieval

### Technical Details
- Added `import NetworkExtension` to iOS plugin
- `NEHotspotNetwork.fetchCurrent()` is available from iOS 13+ and works on all supported versions (iOS 15+)
